...
<CsOptions>
-odac -iadc -M0 -b16 -B2056
</CsOptions>

<CsInstruments>

;Code recycled from Arbeidskrav 4, then adjusted to fullfil the requirements of Arbeidskrav 7.
...
instr Input
    ...
    ;Analyze pitch and amplitude from microphone input, store data in variables.
    kCps, kRMS pitchamdf gaInput, 500, 2000
    kCps port kCps, 0.05

    ;Failsafe to ensure we don't exceed 1 (as 0dbfs = 1)
    if kRMS > 0.25 then
        gkMicAmp = 1
    else
        ;Multiply the RMS value to a value more useful for amplitude control
        gkMicAmp = kRMS*4
    endif


    ;Scale the CPS value so that 1000Hz = 1. This makes the output value more useful for pitchbend.
    ;Additionally, if the amplitude into the mic is not loud enough, then set the pitchbend factor to 1.
    if gkMicAmp < 0.1 then
        gkScaled = 1
    else
        gkScaled = kCps / 1000
    endif
...
;Drone synth
instr Drone
    ;Whistle into the microphone to bend the pitch
    kCps = 60*gkScaled
    kAmp = 0.1+gkMicAmp
    ...
    ;Sawtooh fundamental // The amplitude of the mic input control the amplitude of the drone.
    aOsc1 vco2 kAmp, kCps-2, 4, 0.01, 0
    aOsc1 *= aEnv

    ;Slow and minor pitch vibrato, move up an octave
    kLFO2 lfo 0.02, 0.2
    kLFO2 += -1
    kModPitch = (kCps*2)*kLFO2

    ;Detuned saw
    aOsc2 vco2 kAmp/2, kModPitch, 4, 0.1, 0.2
    aOsc2 *= aEnv
...
instr Output
    ...
    fout "arbeidskrav_7.wav", 14, gaMasterL, gaMasterR
